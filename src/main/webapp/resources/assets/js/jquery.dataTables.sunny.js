...
(/** @lends <global> */function( $ ) {
	...
	function _fnBuildAjax( oSettings, data, fn )
	{
		...
		var baseAjax = {
			...
			"error": function (xhr, error, thrown) {
				...
				$.sunny.util.safe.handleError(xhr);
	...
	/**
		...
		if(settings.ajax.baseParams!=null){
			 
			$.extend(true,d, settings.ajax.baseParams);
		}
		...
		if(settings.ajax.reParams != null){
			
			$.extend(d, settings.ajax.reParams);	
		}
		
		if (settings.risenFilter!=null){
			
			if(d.filter == null){
				
				d.filter = [];
			}	
			$.merge(d.filter,settings.risenFilter);
		}
		...
		if ( features.bSort ) {
			$.each( sort, function ( i, val ) {
				//d.order.push( { column: columns[val.col].data, dir: val.dir } );
				d.sort = columns[val.col].data;
				d.dir = val.dir==null? 'desc' : val.dir;
	...
	/**
	 * 
	 * only supply one column order
	 * yueguan_flying@qq.com
	 * __reloadData( settings, resetPaging===false, callback, opt);
	 */
	var __reloadData = function ( settings, holdPosition, callback, opt) {
		
		if(opt != null){
			
			if(opt.order!=null){
				
				settings.aaSorting[0] = opt.order;
			}
			
			if(opt.filter!=null){
				
				settings.risenFilter = opt.filter;
			}
			
			if(opt.params!=null){
				
				settings.ajax.reParams = opt.params;
			}
		}
		
		__reload(settings, holdPosition, callback);
	};
	 ...
	 * @returns {DataTables.Api} this
	 */
	_api_register( 'ajax.reloadData()', function ( opt, resetPaging, callback) {
		return this.iterator( 'table', function (settings) {
			__reloadData( settings, resetPaging===false, callback, opt);
		} );
	} );
