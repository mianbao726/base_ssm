...
<jsp:param name="title" value="课时记录管理" />
<jsp:param name="link1" value="课时记录管理" />
                 ...
                 "columnDefs": [
                    {
                     "render": function(data, type, row){
               			 return '<input class="btn btn-xs btn-danger" type="button" value="删除" onclick="delRole(\''+row.id+'\')">'                        	 
                     },
                     "orderable": false,
                     "targets": 2
                 }]
...
// 		window.location.href="ADDROLE.jsp?operate="+operate+"&id="+id;
	...
	function delRole(id){
		var params = {id: id};
		$.sunny.confirm("确认删除选中课时记录? ", function(result) {
			if(result) {
				$.sunny.ajax({
